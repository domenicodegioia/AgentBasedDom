...
def minimum_remaining_values(problem, state):
    ...
    @return: a variable
    """
    return min(problem.assignable_variables(state), key=lambda v: len(problem.legal_assignments(state, v)))
...
def least_constraining_value(problem, state, variable):
    ...
    return sorted(assignable_values,
                  key=lambda v: -sum([len(problem.legal_assignments(problem.assign(state, variable, v), var))
...
class BackTracking:
    ...
    def forward_checking(self, state, domains):
        ...
        for var in self.problem.variables:
            new_domains[var] = self.problem.legal_assignments(state, var)
        return new_domains
